...
const UniswapV2Interface = () => {
  ...
  // Verify contract is deployed and valid
  const verifyContract = async (address, contractType) => {
    try {
      const code = await provider.getCode(address);
      if (code === '0x') {
        throw new Error(`${contractType} contract not found at ${address}`);
      }
      return true;
    } catch (err) {
      console.error(`Contract verification failed for ${address}:`, err);
      throw new Error(`Unable to verify ${contractType} contract. Please check the network.`);
    }
  };
  ...
  const executeSwap = async () => {
    ...
    try {
      ...
      setSuccessMsg('Verifying contracts...');
      await verifyContract(config.routerAddress, 'Router');
  ...
  const createPairAndAddLiquidity = async () => {
    ...
    try {
      ...
      setSuccessMsg('Verifying contracts...');
      await verifyContract(config.factoryAddress, 'Factory');
      await verifyContract(config.routerAddress, 'Router');
